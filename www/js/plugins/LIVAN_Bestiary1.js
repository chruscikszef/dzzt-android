...
(function() {
    ...
    // --- Automatyczne dodawanie po walce ---
    ...
    Window_BestiaryStatus.prototype.refresh = function() {
        ...
        var imgAreaHeight = 180; // Maksymalna wysokość obszaru na obrazek
        ...
        bitmap.addLoadListener(function() {
            ...
            // --- Logika Skalowania ---
            // Obliczamy stosunek dostępnego miejsca do oryginalnego rozmiaru
            var scaleW = w / bw;
            var scaleH = imgAreaHeight / bh;

            // Wybieramy mniejszy stosunek, aby obrazek zmieścił się w obu wymiarach.
            // Math.min(1.0, ...) zapewnia, że nigdy nie powiększymy obrazka, tylko pomniejszymy.
            var scale = Math.min(1.0, scaleW, scaleH);

            // Obliczamy docelowe wymiary
            var dw = Math.floor(bw * scale);
            var dh = Math.floor(bh * scale);

            // Centrowanie przy użyciu nowych, przeskalowanych wymiarów (dw, dh)
            var drawX = (w - dw) / 2;
            var drawY = imgAreaY + (imgAreaHeight - dh) / 2;

            // Rysowanie z przeskalowaniem (source -> destination)
